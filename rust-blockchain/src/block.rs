...
impl Block {
    ...
    pub fn new_genesis_block(coinbase: Transaction) -> Self {
        Self::new_block(vec![coinbase], String::new(), 0).unwrap()
    }

    pub fn run_proof_if_work(&mut self) -> Result<()> {
        info!("Mining ....");

        while !self.valid().unwrap() {
            self.nonce += 1;
        }
    ...
    fn valid(&self) -> Result<bool> {
        ...
        vector.resize(4, '0' as u8);
        Ok(&hasher.result_str()[0..TARGET_HEXT] == String::from_utf8(vector)?)
    }
